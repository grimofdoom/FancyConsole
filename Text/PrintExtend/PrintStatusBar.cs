...
namespace FancyConsole.Text {
    public static partial class Print {
        ...
        public static string BasicStatusBar(int length, int lengthTotal) {
        ...
        public static string BasicStatusBar(int amount, int amountMax, int barLength) {
            int filledLength = (int)((amount / (double)amountMax) * barLength);
            return BasicStatusBar(filledLength, barLength);
        }

        /// <summary>A string of status bar that fades from solid █▓▒░ to empty ' ' based on length</summary>
        public static string FadedStatusBar(int length, int lengthTotal, int fadeSize = 3) {
            if (lengthTotal <= 0)
        ...
        public static string FadedStatusBarScaled(int length, long partialAmount, long fullAmount, int fadeSize = 3) {
            if (fullAmount <= 0) return FadedStatusBar(0, length, fadeSize);
            ...
            return FadedStatusBar(scaledPartial, length, fadeSize);
        }
    }
}